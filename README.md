...
- AI 把内部服务调用改成了外部地址，或把内外部寻址规则混在一起。
- 第三方接口少传、错传字段，或把参数顺序、字段来源理解错。
- 新代码绕过了旧的 `Client`、`Service`、`Helper`、`Adapter` 约定。
- 租户字段、状态字段、权限线索在改动中丢失。
- 响应字段映射发生细微变化，例如 `fireEvent.count.value` 从一个 getter 变成另一个 getter。
- 新增逻辑影响了旧调用链，但受影响入口、调用方和测试范围不清楚。
- 需求、设计、任务列表和实际提交对不上。
...
- 项目既有 Git，也可能有 SVN，或者只能提供改动前后的目录快照。
- 需求来自 OpenSpec、spec-kit、superpowers、普通 Markdown 或用户自己的任务清单。
- 业务里存在隐式规则，例如内部寻址、租户隔离、状态流转、第三方参数契约。
- 你希望把旧代码里已有的调用形态先提取成候选契约，再人工确认是否作为本次审计标准。
- Java 项目依赖不完整，仍希望用 Spoon 的 `NOCLASSPATH` 模式做 AST 级审查。
