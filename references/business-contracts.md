...
JSON 契约是期望结果，`--response-snapshot` 必须是接口真实运行后得到的实际响应。不得把同一文件或内容完全相同的复制文件同时作为两种输入；工具会将其记录为输入角色冲突并拒绝比较。
...
每条未检查契约会生成 `manual_review_obligations`。工具从契约文本提取接口名、方法名和字段名，并反查生产源码候选位置，供审查 AI 做契约到代码的人工交叉核验。
